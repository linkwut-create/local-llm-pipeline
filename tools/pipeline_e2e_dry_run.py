...
def step_execute(task_id: str, route: dict, use_mocks: bool = True) -> dict:
    """Execute workers based on route tier. Mock mode for tests, real by default.

    Tier dispatch:
      - local -> llama.cpp via LiteLLM (local_llm_worker)
      - flash -> DeepSeek-V4 Flash API (deepseek_client)
      - pro   -> DeepSeek-V4 Pro API  (deepseek_client)
      - blocked / ask_user -> no execution
    """
    import json
    import os
    ...
    tier = route.get("tier", "local")
    cloud_model = route.get("cloud_model", "")
    results = []

    # Blocked / ask_user - no execution
    if route_type == "blocked":
        results.append({"step": "execute", "status": "blocked",
                        "tier": tier, "cloud_model": cloud_model,
                        "reason": "Route is blocked - no execution allowed."})
        return {"ok": True, "route_type": route_type, "tier": tier, "results": results}
    ...
    if use_mocks:
        ...
        return {"ok": True, "route_type": route_type, "tier": tier,
                "cloud_model": cloud_model, "results": results}

    # --- Real dispatch ---
    task_desc = route.get("reason", task_id)

    if tier == "local":
        try:
            from local_llm_worker import WorkerConfig, call_openai_compat
            config = WorkerConfig(
                model="qwen3-coder-30b",
                provider="openai-compatible",
                base_url=os.environ.get("LOCAL_LLM_BASE_URL", "http://127.0.0.1:4000/v1"),
                api_key=os.environ.get("LOCAL_LLM_API_KEY", "sk-zero12-cluster"),
                max_output_tokens=16384,
                timeout=1000,
            )
            system = "You are a code review assistant. Be thorough and specific."
            user = "Review the following task and produce a structured analysis:\n\nTask: " + task_desc
            result = call_openai_compat(system, user, config)
            save_artifact(task_id, "local_review.json",
                          json.dumps({"content": result.content, "usage": result.usage}),
                          artifact_type="review", tool_name="local_worker",
                          creator="qwen3-coder-30b")
            results.append({"artifact": "local_review", "status": "saved",
                            "model": config.model, "tier": "local"})
        except Exception as e:
            results.append({"artifact": "local_review", "status": "failed",
                            "error": str(e)[:200]})
        ...
        try:
            ...
            resp = call_deepseek(
                prompt="Analyze this task and produce a patch candidate:\n\n" + task_desc,
                model="deepseek-v4-flash",
                thinking=False,
                reasoning_effort="low",
            )
            if resp["ok"]:
                save_artifact(task_id, "flash_review.json",
                              json.dumps({"content": resp["content"], "usage": resp["usage"]}),
                              artifact_type="review", tool_name="flash_worker",
                              creator="deepseek-v4-flash")
                results.append({"artifact": "flash_review", "status": "saved",
                                "model": "deepseek-v4-flash", "tier": "flash"})
            else:
                results.append({"artifact": "flash_review", "status": "failed",
        ...
        try:
            from deepseek_client import call_deepseek
            resp = call_deepseek(
                prompt="Deep analysis and adjudication of this task:\n\n" + task_desc,
                model="deepseek-v4-pro",
                thinking=True,
                reasoning_effort="high",
            )
            if resp["ok"]:
                save_artifact(task_id, "pro_adjudication.json",
                              json.dumps({"content": resp["content"], "usage": resp["usage"]}),
                              artifact_type="adjudication", tool_name="pro_worker",
                              creator="deepseek-v4-pro")
                results.append({"artifact": "pro_adjudication", "status": "saved",
                                "model": "deepseek-v4-pro", "tier": "pro"})
            else:
                results.append({"artifact": "pro_adjudication", "status": "failed",
                                "error": resp.get("error", "unknown")})
        except Exception as e:
            results.append({"artifact": "pro_adjudication", "status": "failed",
                            "error": str(e)[:200]})
