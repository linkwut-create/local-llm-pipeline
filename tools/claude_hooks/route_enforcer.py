...
def get_active_task() -> dict | None:
    """Find the most recent active task session."""
    tasks_dir = _tasks_dir()
    if not tasks_dir.exists():
        return None
    sessions = sorted(tasks_dir.iterdir(), key=lambda p: p.name, reverse=True)
    for sd in sessions:
    ...
    if should_trigger_committee(task_id):
        return {
            ...
            "reason": (
                ...
                f".local_llm_out/tasks/{task_id}/plan.json --json"
            ),
        }
