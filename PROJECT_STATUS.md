...
| P4-B | Not started | Smallest viable implementation slice per `docs/P4_WORKER_POOL_DRY_RUN_PLAN.md` §5. CLI-opt-in JSON probe report in `tools/local_llm_check.py`. Requires separate approval after P4-A. |
| P4-C | Not started, optional | Configurable worker list (env var / JSON). Only after P4-B is in use and a real config need has emerged. |
| P4-D | Not started, optional | Docs/status closeout for the P4 chain (mirrors P3-E pattern). |
