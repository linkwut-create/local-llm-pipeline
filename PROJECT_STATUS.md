...
| P6-B1 | Not started | Timeout observability fix — smallest viable slice per §6 of the audit doc. Scope: fix timeout propagation in `_wrap_worker_call` (C1), connect timeout to health-store penalty, clear stale `last_timeout` after success (H2), focused tests. Requires separate approval after P6-A.1. |
