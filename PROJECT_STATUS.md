...
| P3-C1 | In review | First behavioral flip: `confidence=="low"` auto-escalation now default OFF in `_check_quality_escalation`; legacy behavior restorable via `LOCAL_LLM_AUTO_ESCALATE_ON_LOW_CONFIDENCE=true`. `_derive_escalation_trigger` updated in lock-step so the ledger `escalation_trigger` label matches the branch that actually fires (returns `"low_confidence"` only when knob is ON; falls through to `"uncertain_points"` / `"unknown"` otherwise). `tests/test_p3_env_knobs.py` expanded to 72 tests covering default-OFF, env-knob restore (truthy values), falsy/empty/unrecognized → OFF, dual-signal fallthrough to uncertain branch, and three untouched-path guards (uncertain branch + timeout downgrade). `tests/test_mcp_escalation_ledger_env.py` and `tests/test_layer4_quality.py::TestQualityEscalation` gained autouse fixtures that set both knobs to `true`, preserving the legacy plumbing tests' intent. `_check_quality_escalation` `uncertain_points > 3` and `timeout` branches unchanged. No `call_ledger.py` / `call_ledger_cli.py` / `local_llm_profiles.json` / `CLAUDE.md` / `docs/mcp-task-policy.md` / `VERSION` / tag changes. |
