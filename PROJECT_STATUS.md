...
## Advisor Output Quality (Post-v0.12.0, Q-chain, 2026-05-26) — Closed
...
| Z-3 | Done (`3a84077`, `37759c9`) | CLI-only cost savings report. Added `tools/cloud_cost_reference.json` (5 tiers, 22 profile mappings), savings helpers in `tools/call_ledger.py` (`load_cloud_rates`, `resolve_savings_tier`, `compute_savings`, `savings_for_group`, `build_savings_report`), `savings` CLI subcommand in `tools/call_ledger_cli.py` with `--by total/task/profile/model/backend/mcp-tool/project/location`. Formula: `cloud_equivalent - actual_cost`, advisory-only, not-for-billing. Z-3.0 fix (`37759c9`): alias mapping `task→task_type`, `location→execution_location`, `mcp-tool→extra.mcp_tool_name/tool_name`. 7 --by dimensions verified on real ledger (3,124 calls, 14.3M tokens, 26.30 CNY cloud equivalent). 28 savings-focused tests, 215 call_ledger, 2084 full suite. No ledger mutation, no write schema change. VERSION `0.12.0` unchanged. MCP tool count remains 12. |
| Z-4 | Done (`8a6ff00`) | CLI-only cross-project feedback ledger. Added `tools/feedback_ledger.py` (390 lines) — manual CLI-only append-only JSONL feedback ledger with subcommands `record`, `summary`, `by-target`. Record schema: 15 fields. Disposition enum: `accepted`, `rejected`, `false_positive`, `converted_to_fix`, `deferred`. Suggestion types: `review_flag`, `test_gap`, `docs_gap`, `risk_note`, `refactor_suggestion`, `baseline_audit`, `cost_finding`, `quality_finding`. Privacy: FORBIDDEN_KEYS + pattern redaction (API keys, tokens, private keys), field length truncation. Runtime records under `.local_llm_out/feedback/`. Added `tests/test_feedback_ledger.py` (402 lines, 35 tests) — all mocked with temp synthetic JSONL files. No MCP tool registration, no hooks/gates/CI, no auto-write. Full suite 2119 passed. VERSION `0.12.0` unchanged. MCP tool count remains 12. |
