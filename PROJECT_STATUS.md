...
| P3-B | In review | Env knob helper + constants in `tools/local_llm_mcp_server.py`: `_ENV_AUTO_ESCALATE_ON_LOW_CONFIDENCE`, `_ENV_AUTO_ESCALATE_ON_UNCERTAIN`, `_parse_env_flag(name, default=False)`. **Plumbing only — not wired into `_check_quality_escalation`.** Pre-P3 runtime behavior preserved unchanged: `confidence=="low"` and `len(uncertain_points) > 3` still auto-escalate regardless of either knob's value. New `tests/test_p3_env_knobs.py` (49 tests) covers the parser semantics (unset/truthy/falsy/empty/unrecognized/case+whitespace) plus three runtime-invariance smoke tests that fail if the wiring leaks into this phase. Behavioral flip remains scheduled for P3-C1 / P3-C2. |
