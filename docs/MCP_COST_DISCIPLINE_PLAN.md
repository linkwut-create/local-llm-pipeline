...
| **P5** | V4-Flash local experimental profile. Add profile entry in `local_llm_profiles.json`. Manual invocation only. Ledger records provider=auto-detected (Ollama or OpenAI-compatible), model=v4-flash (placeholder). | `local_llm_profiles.json` | low |
